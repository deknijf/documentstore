...
class VDKBankClient:
    """
    Read-only VDK client.
    IMPORTANT: this client only uses GET requests to bank endpoints.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if not self.base_url:
            raise RuntimeError("VDK base URL ontbreekt")
            ...
            out.append(
                {
                    ...
                    "name": str(row.get("name") or row.get("product") or external_id).strip(),
    ...
    def _normalize_transactions(self, payload: Any) -> list[dict[str, Any]]:
        ...
        for row in rows:
            ...
            counterparty = (
                row.get("debtorName")
                or row.get("creditorName")
                or row.get("counterpartyName")
                or ""
            )
            external_tx_id = str(
                row.get("transactionId")
                or row.get("entryReference")
                or row.get("id")
                or ""
            ).strip()
